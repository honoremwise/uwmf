...
$(document).ready(function(){
  ...
  function getbookvalidation(){
    var bookcode=$("#newbookcode");var bookcategory=$("#libcatenameid").val();var bookname=$("#libookname").val();var author=$("#bookwriterid").val();
    var version=$("#bookversionid").val();var price=$("#priceid").val();var currency=$("#pricecurrencyid").val(); var publication=$("#publicationid").val();
    //check for empty data in the inputs
    var element=[bookcode,bookcategory,bookname,author,version,price,currency,publication];
    var tests=[];
  ...
  function getborrowvalidate(){
    ...
    var month=$("#monthid").val(); var year=$("#yearbackid").val();
    var elements=[bookcode,student,dateback,month,year];
    var tests=[];
    for (var i = 0; i < elements.length; i++) {
      if (elements[i].length>0) {
        continue;
