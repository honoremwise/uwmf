...
class Uploads extends CI_Controller
{
  ...
  public function saveid() //upload id/passport
  {
    if (count($_FILES)==1){
      ...
      if ($this->form_validation->run()==TRUE && $_FILES[$fileuse]['size']<=100000){
        ...
        if ($reference!=false) { //correct reference number
          ...
          if (file_exists($checkExisting)){
            ...
            $this->savefile($save,$file);
  				...
  				$data=array(
  					'error'=>'Invalid upload, try again and upload a file of less than 1Mb',
  				);
  ...
  public function checkfile(){
    if (count($_FILES)==1) {
      ...
      if (isset($_FILES[$name]['name']) && $_FILES[$fileuse]['size']<200000){
        $mime=get_mime_by_extension($_FILES[$name]['name']);
        if (isset($_FILES[$name]['name']) && $_FILES[$name]['name']!="") {
          if (in_array($mime,$allowed_mime_type_arr)) {
            return true;
          }else{
            $this->form_validation->set_message('checkfile', 'Please select only  file of type pdf or image files');
            return false;
          }
        } else {
    ...
    if ($save==FALSE) {
      ...
      if (strpos($file,"identity")) {
        ...
        if ($save!=false){
          ...
          $this->load->view('views_pages/bachelor_master_program_uploads',$data);
        }else{
          $data=array(
            'error'=>'File upload error, try again',
          );
