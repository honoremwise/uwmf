 ...
 */
class TranscriptFiles extends CI_Controller
{
  ...
  public function checkUploadne(){
    ...
    if (isset($_FILES['candidatereport']['name'])) {
      $mime=get_mime_by_extension($_FILES['candidatereport']['name']);
      if (isset($_FILES['candidatereport']['name']) && $_FILES['candidatereport']['name']!="") {
  ...
  public function saveFiles()
  {
    ...
    $file=$_FILES['candidatereport']['name'];
    $this->form_validation->set_rules('candidatereport', '', 'callback_checkUploadne');
    if ($this->form_validation->run()==TRUE) {
      $user=$_SESSION['username'];
      $type= pathinfo($_FILES['candidatereport']['name'], PATHINFO_EXTENSION);
      $config["file_name"] = $reference."_candidatereport.".$type;
      $file= $reference."_candidatereport.".$type;
      ...
      $save = $this->upload->do_upload('candidatereport');
      if ($save==FALSE) {
  ...
  public function savefaith()
  {
    ...
    $config['max_size']      =2000;
    $file=$_FILES['candidatereport']['name'];
    $this->form_validation->set_rules('candidatereportfaith', '', 'callback_checkUploadne');
    if ($this->form_validation->run()==TRUE) {
      $user=$_SESSION['username'];
      $type= pathinfo($_FILES['candidatereport']['name'], PATHINFO_EXTENSION);
      $config["file_name"] = $reference."_candidatereportfaith.".$type;
      $file= $reference."_candidatereportfaith.".$type;
      $this->load->library('upload', $config);
      $checkExisting=$_SERVER['DOCUMENT_ROOT'].'/real/profiles/'.$file;
      if (count($checkExisting)!=0) {
        //delete the file
  ...
  public function studentidentity()
  {
    ...
    if ($this->form_validation->run()==TRUE) {
      ...
      if ($reference!=false) { //correct reference number
        $type= pathinfo($_FILES['candidatereport']['name'], PATHINFO_EXTENSION);
        $config["file_name"] = $reference[0]."_candidatereport.".$type;
        $file= $reference[0]."_candidatereport.".$type;
        $this->load->library('upload', $config);
        $checkExisting=$_SERVER['DOCUMENT_ROOT'].'/real/profiles/'.$file;
        if (count($checkExisting)!=0) {
          //delete the file
