 ...
 */
class IdentityFiles extends CI_Controller
{
  ...
  public function checkUploadne(){
    ...
    if (isset($_FILES['identity']['name'])) {
      $mime=get_mime_by_extension($_FILES['identity']['name']);
  ...
  public function saveFiles()
  {
    ...
    $this->form_validation->set_rules('identity', '', 'callback_checkUploadne');
    if ($this->form_validation->run()==TRUE) {
      $user=$_SESSION['username'];
      $type= pathinfo($_FILES['identity']['name'], PATHINFO_EXTENSION);
      $config["file_name"] = $reference."_identity.".$type;
      $file= $reference."_identity.".$type;
      ...
      $save = $this->upload->do_upload('identity');
      if ($save==FALSE) {
  ...
  public function studentidentity()
  {
    ...
    if ($this->form_validation->run()==TRUE) {
      ...
      if ($reference!=false) { //correct reference number
        $type= pathinfo($_FILES['identity']['name'], PATHINFO_EXTENSION);
        $config["file_name"] = $reference[0]."_identity.".$type;
        $file= $reference[0]."_identity.".$type;
        $this->load->library('upload', $config);
        $checkExisting=$_SERVER['DOCUMENT_ROOT'].'/real/profiles/'.$file;
        if (count($checkExisting)!=0) {
          //delete the file
